...
class HypergraphConv(MessagePassing):
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor, x2: Tensor =None,
        ...
        # out = F.dropout(self.FFN_1(residual), p=self.dropout, training=self.training)  
        # out = self.layer_norm(out + residual)

        # residual = out
        # out = F.dropout(self.FFN_1(out), p=self.dropout, training=self.training)

        residual = self.layer_norm(out + residual)
        out = self.FFN_2(F.gelu(self.FFN_1(residual)))
