...
class HypergraphConv(MessagePassing):
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor, x2: Tensor =None,
        ...
        if x2 is not None:
            ...
            alpha2 = (x2_i * self.att).sum(dim=-1)
        ...
        if x2 is not None:
            ...
            hyperedge_attr = out + out2
            hyperedge_attr = self.layer_norm(hyperedge_attr.view(-1, self.heads * self.out_channels))
            ...
            alpha2 = (torch.cat([x2_i, x_j], dim=-1) * self.att2).sum(dim=-1)
            alpha_mix = torch.cat([alpha, alpha2], dim = 0)
            alpha_mix = F.leaky_relu(alpha_mix, self.negative_slope)
            h_edge_index = torch.cat([hyperedge_index[0],hyperedge_index[0]], 0)
            alpha_mix = softmax(alpha_mix, h_edge_index, num_nodes=x.size(0))
        ...
        residual = self.layer_norm(out + residual)
        out = self.FFN_2(F.relu(self.FFN_1(residual)))
